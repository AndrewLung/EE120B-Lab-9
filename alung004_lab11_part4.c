 ...
 *
 *  Demo Link: https://www.youtube.com/watch?v=zqOpblbCdaA&feature=youtu.be
 */
#include <avr/io.h>
...
static unsigned short output = 0xFF;
...
void transmit_data(unsigned short data) {
	int i;
	for (i = 0; i < 16; ++i) {
		PORTC = 0x08; //SRCLR: 0x08 for reg1 0x10 for reg2
		PORTC |= ((data >> i) & 0x01); // SER = next bit of data to be sent
		PORTC |= 0x02; // SRCLK
	}
	PORTC |= 0x04; // RCLK, stores values
	PORTC = 0x00;
}
